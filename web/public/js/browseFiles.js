(function ($) {
    ...
    $.fn.fileViewer.defaults.viewers = {
        ...
        csv: {
            ...
            render: function (file) {
                return `<div id="CSVTable" class="${this.class}"></div> 
                <script>

  $('#CSVTable').CSVToTable('${  file.path }',{loadingText:'<div id="loading_table" ><i class="fa-solid fa-spinner fa-spin-pulse"></i></div>'});
        ...
        default: {
            ...
            class: 'default-viewer',
            render: function (file) {
                return '<span id="' + this.id + '" class="' + this.class + '">Formato não suportado!!</span>';
            }
        }
    }
...
class browse {
    constructor(files,path){
        if(files == undefined||files.length == 0) {
            alertar("Opa !","Não existem arquivos anexados para esse registro.")
            return
        }
        $("#filesBrowser").remove()
        ...
        $("body").append(`
            ...
            ${files.map(element => {
                return `
                <div path="${path.replace("public/","")}" file="${element}" class='file'>
    ...
    getFileClick(){
        $("body").on("click",".file",function(){
            ...
            $('#previewArea').fileViewer({
                filePath: path+file,
                fileName: file} )
        })
        $("#filesArea .file")[0].click()
    }
    getOnClose(){
